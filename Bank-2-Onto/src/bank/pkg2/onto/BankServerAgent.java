            ...
            switch (msg.getPerformative()) {
               ...
               case (ACLMessage.REQUEST):
                  ...
                  /*else if (action instanceof Transfer)
                      addBehaviour(new HandleTransfer(myAgent,msg));*/
...
// ------------------------------------------------  Handler for an Operation request
   ...
   class HandleTransfer extends OneShotBehaviour{
       private ACLMessage request;

      HandleTransfer(Agent a, ACLMessage request) {
      ...
      public void action() {
         ...
         try {
            ContentElement content = getContentManager().extractContent(request);
            Transfer tf = (Transfer)((Action)content).getAction();
            ACLMessage reply = request.createReply();
            Object obj = processTransfer(tf);
            if (obj == null) replyNotUnderstood(request);
            else {
               reply.setPerformative(ACLMessage.INFORM);
               Result result = new Result((Action)content, obj);
...
// -------------------------------------------
      ...
      if (mo.getType() != DEPOSIT && mo.getType() != WITHDRAWAL && mo.getType() != TRANSFERENCE)
         return null;
      if (mo.getType() == DEPOSIT)
         acc.setBalance(acc.getBalance() + mo.getAmount());
      else if (mo.getType() == TRANSFERENCE)
          acc.setBalance(acc.getBalance() + mo.getAmount());
   ...
   Object processTransfer(Transfer tf){
       Account acc2 = (Account)accounts.get(tf.getTargetAccount());
       
      
      if (acc2 == null) return newProblem(ACCOUNT_NOT_FOUND);
      
      if (tf.getAmount() <= 0) return newProblem(ILLEGAL_OPERATION);
      if (tf.getType() != TRANSFERENCE)
         return null;
      else if(tf.getType() == TRANSFERENCE)
          acc2.setBalance(acc2.getBalance() + tf.getAmount());
      Operation op = new Operation();
      op.setType(tf.getType());
      op.setAmount(tf.getAmount());
     op.setAccountId(tf.getTargetAccount());
      op.setDate(new java.util.Date());
      List l = (List)operations.get(acc2.getId());
      l.add(op);
      operations.put(acc2.getId(), l);
      return acc2;
   }
